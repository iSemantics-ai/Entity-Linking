...
class SimCSE_Matcher(object):
    ...
    def search(self, queries: Union[str, List[str]], 
        ...
        if not self.is_faiss_index:
            ...
            logger.info("using similarity method")
            ...
            def pack_single_result(dist, idx):
                results = [(self.index["ids"][i], self.index["sentences"][i], s) for i, s in zip(idx, dist) if s >= threshold]
                return results
    ...
    def match_data(self,
        ...
        @params
        -------
        source_text(List[str]): List of names to search inside
        query_text(str): List of required named to detect it's matches
        ...
        """
        if not isinstance(source_text,list) and isinstance(query_text,list):
            raise ValueError("source_text and query_text must be list of str!!!")

        # Calc number of branches
        b_number = math.ceil(len(source_text) / b_size)
        # Initialize the search candidates dictionary
        candidates = {k:[] for k in query_text }
        # Encode the queries
        query_vecs = self.encode(query_text,normalize_to_unit=True, keepdim=True, return_numpy=True)
        
        # Loop over all branches
        for b_index in range(b_number):
            logger.info("processed chunks {}/{}".format(b_index,b_number))
            # Define start and end indecies
            start = b_index * b_size
            end = start+b_size 
            if b_index == b_number-1:
                text = source_text[start:]
                ids = source_ids[start:]
            ...
            self.build_index(text, ids)
            # Search in the faiss matrix for most simialr vectors to the query
            results =  self.search(query_vecs, top_k=top_k, threshold=threshold)
            logger.info("search completed")
            # Fill the dict of canditatesPR
            for i,k in enumerate(candidates.keys()): 
                candidates[k] += results[i]
        
        logger.info("sorting the scores")
        ...
        for k,v in candidates.items():
            ...
            if len(v) == 0:
                ...
                match_text.append(None)
                score.append(None)
                matches.append(None)
                continue 
            # Set each query with it's matches as one row in the frame
            ids = np.array([r[0] for r in v])
            names = np.array([r[1] for r in v])
            
            scores = np.array([r[2] for r in v])
            ...
            match_text.append(names[0]) 
            score.append(scores[0])
            matches.append(list(zip(names[:top_k], scores[:top_k])))
        d_results = pd.DataFrame({'query':list(candidates.keys()), 'match_text':match_text, 'source_id':match_ids , 'score':score, 'matches':matches})
